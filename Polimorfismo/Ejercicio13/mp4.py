...
class Mp4:
    ...
    def __add__(self, nueva_cancion):
        for i in range(len(self.__canciones[0])):
            if(nueva_cancion[0]==self.__canciones[0][i] and nueva_cancion[1]==self.__canciones[1][i]):
    ...
    def __sub__(self, nuevo_video):
        for i in range(len(self.__videos)):
            if(nuevo_video[0]==self.__videos[i][0] and nuevo_video[1]==self.__videos[i][1]):
                self.__videos[i][0] = []
                self.__videos[i][1] = []
                self.__videos[i][2] = []
...
reproductor = Mp4("Toshiba", 123.66, 2, [["back To Black", "Lost On You"], 
                                    ["Amy Winehouse", "LP"], 
                                    [100, 150]],
                                    1,
